...
internal class TeamControllerTests {
    ...
    fun `when creating teams it delegates to TeamRepository`() {
        every { teamRepository.save("first-team-name") } returns "first-team-name"
        ...
        verify(exactly = 1) {
            teamRepository.save("first-team-name")
        }
    }

    @Test
    fun `when getting teams it delegates to TeamRepository`() {
        every { teamRepository.findAll() } returns listOf("first-team-name", "second-team-name")
